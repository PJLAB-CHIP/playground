...
#define HGEMM_CHECK_CUDART_ERROR(_expr_)                                       \
    do {                                                                       \
        ...
        if (HGEMM_UNLIKELY(_ret_ != cudaSuccess)) {                            \
            ...
            HLOG("CUDA Runtime API error = %04d \"%s\", runtime version: %d, " \
                 "driver version: %d",                                         \
                 static_cast<int>(_ret_), _err_str_, _rt_version_,             \
                 _driver_version_);                                            \
            exit(EXIT_FAILURE);                                                \
        }                                                                      \
    } while (0)
...
#define HGEMM_CHECK_CUBLAS_ERROR(_expr_)                                       \
    do {                                                                       \
        ...
        if (HGEMM_UNLIKELY(_ret_ != CUBLAS_STATUS_SUCCESS)) {                  \
            ...
            HLOG("CUBLAS API error = %04d, runtime version: %zu",              \
                 static_cast<int>(_ret_), _rt_version_);                       \
            exit(EXIT_FAILURE);                                                \
        }                                                                      \
    } while (0)
