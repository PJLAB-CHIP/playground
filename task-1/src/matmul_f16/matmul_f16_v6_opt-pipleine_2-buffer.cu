...
{
__global__ void mmaAsyncStage2Kernel(const half* __restrict__ A, const half* __restrict__ B,
...
size_t initMmaAsyncStage2()
{
    ...
    HGEMM_CHECK_CUDART_ERROR(cudaFuncSetAttribute(
        mmaAsyncStage2Kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

template <>
void matmul<float16_t, 6>(const size_t m, const size_t n, const size_t k, const float16_t* const A,
