...
size_t initWmmaAsync()
{
    ...
    HGEMM_CHECK_CUDART_ERROR(cudaFuncSetAttribute(
        hgemmV4_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

template <>
void matmul<float16_t, 4>(const size_t m, const size_t n, const size_t k, const float16_t* const A,
                        const float16_t* const B, float16_t* const C)
{
    static size_t smem_max_size = initWmmaAsync();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, ceilDivide(m, BLOCK_ROWS), ceilDivide(n, BLOCK_COLS * BLOCK_STRIDE));

    hgemmV4_kernel<<<grid, block, smem_max_size>>>(A, B, C, m, n, k);
}
}  // namespace playground
