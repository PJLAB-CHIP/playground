...
__global__ void sgemmV5_kernel(const float32_t* const A, const float32_t* const B,
                               float32_t* const C, const size_t M, const size_t N, const size_t K)
{
    ...
    __shared__ float32_t s_a[2][BK][BM];
    __shared__ float32_t s_b[2][BK][BN];
...

    {
        ...
        rCast<float4>(s_b[0][load_b_smem_k][load_b_smem_n]) = rCast<float4>(r_load_b[0]);
    }

    for (size_t bk = 1; bk < (K + BK - 1) / BK; bk++) {

        size_t smem_sel = (bk - 1) & 1;
        size_t smem_sel_next = bk & 1;
...
#pragma unroll
        for (size_t tk = 0; tk < BK; tk++) {
            rCast<float4>(r_comp_a[0]) = rCast<float4>(s_a[smem_sel][tk][Ty * TM / 2]);
            rCast<float4>(r_comp_a[4]) = rCast<float4>(s_a[smem_sel][tk][Ty * TM / 2 + BM / 2]);
            rCast<float4>(r_comp_b[0]) = rCast<float4>(s_b[smem_sel][tk][Tx * TN / 2]);
            rCast<float4>(r_comp_b[4]) = rCast<float4>(s_b[smem_sel][tk][Tx * TN / 2 + BN / 2]);
...
#pragma unroll
        ...
        rCast<float4>(s_b[smem_sel_next][load_b_smem_k][load_b_smem_n]) =
            rCast<float4>(r_load_b[0]);
...
#pragma unroll
    for (size_t tk = 0; tk < BK; tk++) {
        rCast<float4>(r_comp_a[0]) = rCast<float4>(s_a[1][tk][Ty * TM / 2]);
        rCast<float4>(r_comp_a[4]) = rCast<float4>(s_a[1][tk][Ty * TM / 2 + BM / 2]);
        rCast<float4>(r_comp_b[0]) = rCast<float4>(s_b[1][tk][Tx * TN / 2]);
        rCast<float4>(r_comp_b[4]) = rCast<float4>(s_b[1][tk][Tx * TN / 2 + BN / 2]);
...
template <>
void matmul<float32_t, 5>(const size_t M, const size_t N, const size_t K, const float32_t* const A,
                          const float32_t* const B, float32_t* const C)
{
    constexpr dim3 BlockSize(16, 16);
    constexpr dim3 GridSize(32, 32);
    sgemmV5_kernel<<<GridSize, BlockSize>>>(A, B, C, M, N, K);
}
}  // namespace playground